...
class OsAbstractParser extends ParserAbstract {
	...
	for (let i = 0, l = this.collection.length; i < l; i++) {
	  ...
	  if (match !== null) {
		...
		for (let key in OS_SYSTEMS) {
		  if (String(name).toLowerCase() === String(OS_SYSTEMS[key]).toLowerCase()) {
			name = OS_SYSTEMS[key];
			short = key;
			break;
		  }
  ...
  parsePlatform(userAgent) {
	if (this.getBaseRegExp('arm|aarch64|Watch ?OS|Watch1,[12]').test(userAgent)) {
	  return 'ARM';
	}
	...
	if (this.getBaseRegExp('WOW64|x64|win64|amd64|x86_6').test(userAgent)) {
	  return 'x64';
	}
	if (this.getBaseRegExp('i[0-9]86|i86pc').test(userAgent)) {
	  return 'x86';
	}
