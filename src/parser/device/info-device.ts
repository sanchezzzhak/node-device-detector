...
const castSizeToObject = (size) => {
  const [width, height, thickness] = size.split('x');
...
const getDataByIdInCollection = (collection, id) => {
  const data = collection[parseInt(id)];
  if (data === void 0) {
    return null;
  }
  return data;
  ...
  prepareResultDisplay(result) {
    ...
    if (result.display) {
      ...
      if (typeof resolution !== 'string') {
        const resolutionWidth = parseInt(resolution.width);
        const resolutionHeight = parseInt(resolution.height);
