class DataPacker {
  ...
  static unpack(str, shortKeys) {
    ...
    for (let item of data) {
      let short = item[1];
      let value = item[2] !== void 0 ? item[2] : "";
      let path = shortKeys[short];
      DataPacker.set(obj, path, value);
  ...
  static set(obj, path, value, doNotReplace) {
    ...
    if (typeof path === 'string') {
      return DataPacker.set(obj, path.split('.').map(DataPacker.getKey), value, doNotReplace);
    }
