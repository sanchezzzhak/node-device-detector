...
export class ClientHints {
   ...
   * ```
   */
  static getHeaderClientHints(): any {
    return {
      'accept-ch': [
        CH_UA_FULL_VERSION,
   ...
   * js
   */
  static isSupport(headers: any): boolean {
   ...
   * @private
   */
  #parseHints(hints, result:ResultClientHints): void {
    for (let key in hints) {
      ...
      switch (lowerCaseKey) {
        ...
        case 'fullversionlist':
          ...
          break;
        case 'http-sec-ch-ua':
        // eslint-disable-next-line,@typescript-eslint/ban-ts-comment,no-fallthrough
        case 'sec-ch-ua':
        ...
        case 'sec-ch-ua-full-version-list':
          ...
          while (matches = pattern.exec(value)) {
            ...
            const skip = brand.indexOf('Not;A') !== -1 || brand.indexOf('Not A;') !== -1 || brand.indexOf('Not.A') !== -1;
            if (skip) {
              continue;
            }
   ...
   * @private
   */
  #parseMeta(meta, result: ResultClientHints): void {
    for (let key in meta) {
      let value = meta[key];
  ...
  /**
   * @param {JSONObject|null} hints - headers or client-hints params
   * @param {JSONObject|null} meta  - client custom js metric params
   * @return {ResultClientHints}
   */
  parse(hints, meta = {}): ResultClientHints {
    const result = this.#blank()
